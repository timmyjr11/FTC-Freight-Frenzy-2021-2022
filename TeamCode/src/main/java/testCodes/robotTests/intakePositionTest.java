    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (opModeIsActive()) {
            if (gamepad2.right_trigger >= 0.5) {
                ...
                if (intake.getCurrentPosition() == Math.floor(range * 384.5) || (intake.getCurrentPosition() >= (Math.floor(range * 384.5) - 10) && intake.getCurrentPosition() <= (Math.floor(range * 384.5) + 10))) {
                    intake.setPower(0);
                } else {
                    intake.setPower(-0.3);
                    intake.getCurrentPosition();
